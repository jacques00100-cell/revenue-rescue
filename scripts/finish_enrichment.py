#!/usr/bin/env python3
"""
Process remaining HVAC and salon leads
"""
import json
import re
import subprocess
...
def has_booking(content):
    if not content:
        return False
    c = content.lower()
    return any(x in c for x in ['book', 'schedule', 'appointment', 'booking', 'calendly', 'acuity'])

def has_chat(content):
    if not content:
        return False
    c = content.lower()
    return any(x in c for x in ['live chat', 'chat', 'intercom', 'tawk', 'zendesk', 'drift'])
...
if __name__ == "__main__":
    ...
    # Process HVAC batch2 (remaining)
    print("="*70)
    print("HVAC BATCH 2 - CONTINUING")
    print("="*70)
    ...
    # Process salon (remaining)
    print("\n" + "="*70)
    print("SALON LEADS - CONTINUING")
    print("="*70)
    process_file(f"{base}/salon-leads-dfw.json", "salon")
