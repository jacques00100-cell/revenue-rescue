    ...
    for e in emails:
        ...
        if not any(fp in e_lower for fp in FALSE_PATTERNS):
            if '@' in e and '.' in e.split('@')[-1]:
                domain = e.split('@')[1]
                if len(domain) > 3 and '.' in domain and '.' in domain[-5:]:
...
def get_best_email(emails):
    if not emails:
        return ""
    priority = ['contact@', 'info@', 'hello@', 'support@', 'admin@', 'office@', 'service@', 'repair@']
    for p in priority:
        for e in emails:
            if p in e and 'noreply' not in e:
                return e
    for e in emails:
        if 'noreply' not in e.lower() and '@gmail.com' not in e.lower():
            return e
    for e in emails:
...
def has_booking(content):
    if not content:
        return False
    c = content.lower()
    return any(x in c for x in ['book', 'schedule', 'appointment', 'booking', 'calendly'])

def has_chat(content):
    if not content:
        return False
    c = content.lower()
    return any(x in c for x in ['live chat', 'chat', 'intercom', 'tawk', 'zendesk'])
...
def process_file(filepath, filetype):
    ...
    for i, lead in enumerate(to_process):
        ...
        print(f"[{i+1:3d}/{len(to_process):3d}] {name:<40s} ", end='', flush=True)
        
        try:
            process_lead(lead)
            email = lead['email'][:35] if lead['email'] else "None"
            booking = "Y" if lead['has_online_booking'] else "N"
            print(f"{email:<35s} B:{booking}")
...
if __name__ == "__main__":
    ...
    print("AUTO LEADS")
    print("="*70)
    process_file(f"{base}/auto-leads-dfw.json", "auto")
    
    # Process HVAC batch2
    print("\n" + "="*70)
    print("HVAC BATCH 2 LEADS")
    print("="*70)
    process_file(f"{base}/hvac-leads-dfw-BATCH2.json", "hvac_batch2")
    
    # Process HVAC GMaps
    print("\n" + "="*70)
    print("HVAC GMAPS LEADS")
    print("="*70)
    process_file(f"{base}/hvac-leads-GMAPS.json", "hvac_gmaps")
